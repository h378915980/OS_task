...
* 关于实时时钟更多的资料，那我很感兴趣。这些都是试探出来的，以及看了一些bios 程序，额
*/
...
static void time_init(void)		// 该子程序取CMOS 时钟，并设置开机时间􀃆startup_time(秒)。
{
	struct tm time;
...
"after_page_tables:
	pushl $0		# These are the parameters to main :-) 这些是调用main程序的参数(init/main.c)
	pushl $0
	pushl $0
...
// 机器内存数->memory_end；主内存开始地址 ->main_memory_start；
	...
	time_init();			// 设置开机启动时间􀃎startup_time（见76 行）。
...
// 对于原(父进程)将返回子进程的进程号。所以245-249 句是子进程执行的内容。该子进程
